/**
 * Capture-quality abstention gate (P91 R&D, `ai_outputs/Claude_outputs/output_91.txt`,
 * `docs/DECISIONS.md` D-103). Investigated because the project's two documented catastrophic
 * hard-defect profiles (glare+shadow+blur, partial-shadow+noise — `docs/DECISIONS.md` D-101 §2,
 * reproduced at ~18x corpus scale by P91) drive same-card visual similarity to ~0.10-0.13 while
 * the nearest WRONG card sits at ~0.28-0.41 — a genuine signal inversion no query-side
 * normalization, reference-side augmentation or pooling-representation change this project has
 * tried (P84, P91) can close. A scan this bad is unrecoverable by the matcher; the honest product
 * response is to detect it and ask for a retake BEFORE showing five unrelated cards, not to keep
 * tuning the matcher against an input it structurally cannot read (P91 §38: false confidence is
 ...
 * can run in the browser worker and the offline Node lab — same discipline as rectify.ts and
 * photometric.ts. NOT wired into the shipped scan pipeline by default (matches P84/P85's own
 * ship-tooling-not-behavior precedent) — available as a tested, opt-in check for a future session
 * with the evidence (and ideally a continuous-severity benchmark) to decide whether and how to
 * surface a "try another photo" prompt.
 */
 ...
 *  this does and does not detect. Not wired into the scan pipeline; a future session decides. */
