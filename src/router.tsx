...
const historyRoute = createRoute({
  ...
  // M16 (P51): 'opening' joins the frontend filter set; its backend arm arrives with the
  // integration, so the chip reads an honest empty feed until then.
  validateSearch: (
    search: Record<string, unknown>,
  ): {
    kind?: 'purchase' | 'sale' | 'acquisition' | 'valuation' | 'opening'
    voided?: boolean
  } => ({
    kind: str(search.kind) as
      'purchase' | 'sale' | 'acquisition' | 'valuation' | 'opening' | undefined,
