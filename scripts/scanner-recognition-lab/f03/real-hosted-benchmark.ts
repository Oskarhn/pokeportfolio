...
import { drainAllCardPages } from '../../../src/domain/scanner/index-pagination'
...
const VISUAL_INDEX_INT8_SCALE = 127
...
async function loadProductionIndex() {
  const manifest = JSON.parse(
    await readFile(join(VISUAL_INDEX_DIR, 'manifest.json'), 'utf-8'),
  ) as VisualIndexManifest
  const cardIds = JSON.parse(
    await readFile(join(VISUAL_INDEX_DIR, 'card-ids.json'), 'utf-8'),
  ) as string[]
  const raw = await readFile(join(VISUAL_INDEX_DIR, 'embeddings.bin'))
  ...
  for (let i = 0; i < cardIds.length; i += 1) {
    ...
    for (let d = 0; d < dim; d += 1) vec[d] = (int8[i * dim + d] ?? 0) / VISUAL_INDEX_INT8_SCALE
    vectors.set(cardId, vec)
  }
  return { manifest, cardIds, vectors, dim }
}
...
function searchIndex(
  ...
  const hits: { cardId: string; similarity: number }[] = []
...
async function main() {
  ...
  const { manifest, vectors } = await loadProductionIndex()
  console.log(
    `[f03] production index: ${vectors.size} cards, model ${manifest.modelId}@${manifest.modelRevision}`,
  )

  const restrictedMetadata = metadata.filter((r) => vectors.has(r.id))
  console.log(
  ...
  const report = {
    ...
    productionIndexCardCount: vectors.size,
