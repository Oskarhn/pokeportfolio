// Deterministic synthetic camera-capture distortions — ported from the pre-existing
// scripts/scanner-visual-benchmark/lib/augment.mjs (P76) so P91's results stay directly comparable
// to that project history. Same profile names, same seeding scheme.
...
export async function applyNamedProfile(name, imageBuffer, cardId) {
  const baseSeed = [...cardId].reduce((acc, ch) => (acc * 31 + ch.charCodeAt(0)) | 0, 7)
  const idx = AUGMENTATION_PROFILES.indexOf(name)
  return applyProfile(name, imageBuffer, baseSeed + idx * 97)
}
