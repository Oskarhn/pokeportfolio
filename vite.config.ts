...
export function buildContentSecurityPolicy(
  supabaseUrl: string,
  // P101: Cloudflare Web Analytics is opt-in (VITE_CF_ANALYTICS_TOKEN unset by default), so the
  // policy stays at today's strict shape — no third-party script-src/connect-src host — unless a
  // build actually wires the token in. Defaults to `false` so every existing caller (this test
  // suite included) keeps asserting the analytics-off policy without change.
  analyticsEnabled = false,
): string {
  const origin = new URL(supabaseUrl).origin
  const realtime = origin.replace(/^http/, 'ws')

  // Cloudflare Web Analytics (D-118): the beacon script loads from static.cloudflareinsights.com
  ...
  return {
    ...
    generateBundle() {
      ...
      const csp = buildContentSecurityPolicy(
        supabaseUrl,
        Boolean(process.env.VITE_CF_ANALYTICS_TOKEN),
      )
