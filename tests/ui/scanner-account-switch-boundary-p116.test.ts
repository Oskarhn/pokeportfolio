...
describe('account-switch identity boundary during in-flight analysis (P116 §12)', () => {
  it('25,000 generated switch-timing sequences: an aborted in-flight analysis for A never resolves with usable data, and B always starts from a clean session', async () => {
    await fc.assert(
      fc.asyncProperty(arbSwitchStage, fc.boolean(), async (stage, ocrFirst) => {
        ...
        const catalogDeferred = deferred<CatalogSearchPage>()
