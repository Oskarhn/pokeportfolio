-- M9.1 restates the complete privilege baseline (SECURITY.md §5.9) — required because
-- get_market_movers gained a third argument (p_sort, market_mover_sort — prompt §21), which
-- changes its identity (DROP+CREATE, `20260827120000_m91_market_movers_sort.sql`). Everything else
-- below is identical to `20260826120060_m9_privilege_baseline.sql`; only the one grant line changed.
-- CI selects the lexicographically-latest `*_privilege_baseline.sql` automatically, so this file
-- becoming the newest one is what makes it "the" baseline from here — no CI edit needed. This is
-- also the exact scenario TESTING.md §6a's DROP+CREATE checklist exists for: CI's hostile-grant
-- convergence step re-applies the actual latest baseline migration file, and the *previous* one
-- (M9's) would fail outright trying to grant a signature that no longer exists — caught by CI on
-- the first push of this milestone, which is the reproducibility gate working as intended.
...
-- Signature unchanged by M9/M9.1 — only the body/return columns changed (DROP+CREATE, same arg types).
...
-- M9.1: get_market_movers gained p_sort (market_mover_sort) — new identity, restated here.
...
-- function privileges — unchanged by M9.1.
