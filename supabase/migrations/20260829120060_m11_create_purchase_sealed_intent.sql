-- M11 correction, found by re-reading create_purchase against the new acquisition_lots_check_owner
-- scope check (20260829120000) before this branch shipped: create_purchase's acquisition_lots
-- INSERT never set sealed_intent, which the moved-column trigger now requires to be non-null for
-- any lot on a sealed holding. Every sealed purchase line would have failed with "sealed_intent is
-- required for a lot on a sealed holding" the moment this shipped without this fix. CREATE OR
-- REPLACE, identical signature to M10's — no privilege-baseline churn.
--
-- While fixing this, a real line-level field is added: p_lines[i].sealed_intent (optional, defaults
-- to 'undecided' — the same default add_card_acquisition's direct-add path already uses,
-- 20260829120020), so the purchase builder can capture intent at purchase time instead of every
-- sealed purchase always landing on "undecided" until a separate action changes it.
...
begin
  ...
  for v_idx in 0 .. v_line_count - 1 loop
    ...
    -- New: per-line sealed intent, defaulted the same way add_card_acquisition's direct-add path
