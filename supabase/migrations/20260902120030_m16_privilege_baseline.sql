...
--   maintenance → service_role only; enqueue_portfolio_recompute + m12_*_dirties_history +
--   m12_price_snapshot_enq_* / m12_fx_rate_enq_* → nobody (triggers). The M16 openings_check_owner
--   and the extended acquisition_lots/lot_disposals check-owner trigger bodies join this class:
--   reachable only by their own triggers.
...
-- catalog_sync_runs/watched_card_variants/price_sync_runs/portfolio_recompute_queue/
-- portfolio_recompute_runs get no grant at all, to anon or authenticated — service-only.
