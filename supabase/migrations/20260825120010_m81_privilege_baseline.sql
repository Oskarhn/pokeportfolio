-- M8.1 restates the complete privilege baseline (SECURITY.md §5.9) — required because one new
-- routine exists (remove_holdings_from_portfolio, 20260825120000_m81_void_acquisition_lot_fix.sql).
-- void_acquisition_lot's own signature is unchanged (CREATE OR REPLACE on the same argument types
-- keeps its existing grants), so no re-grant line is needed for it here. CI selects the
-- lexicographically-latest `*_privilege_baseline.sql` automatically, so this file becoming the
-- newest one is what makes it "the" baseline from here — no CI edit needed. No table/column grant
-- changes: M8.1 writes only through columns already named below.
