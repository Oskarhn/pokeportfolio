-- P108: purchase idempotency (docs/BACKLOG.md / P107 §17 finding — PurchaseFormPage/create_purchase
-- had no idempotency key at all, the highest duplicate-write risk of the four money forms: a
-- dropped response after the server has already committed has no mechanism preventing a resubmit
-- from creating a second purchase + acquisition lots).
--
-- Follows create_sale's (20260828120010) and create_opening_from_provisional's (20260902120010)
-- established idempotency shape, adapted for purchase-specific material equivalence:
--   - create_sale: exact-key match replays the existing sale unconditionally — no payload
--     comparison at all. Too permissive for this prompt's contract ("do not silently return the
--     original result for a changed transaction").
--   - create_opening_from_provisional: exact-key match compares a NAMED list of material fields
--     via `IS DISTINCT FROM` and raises a named `idempotency-key-reuse` error on any mismatch.
--     This migration follows that pattern, generalized to a purchase's full material shape.
--
-- MATERIAL EQUIVALENCE (prompt §9-10). A purchase's material facts are: purchased_on, currency,
-- retailer_id, shipping/customs/discount minor, every FX field, and — per line — everything except
-- `lot_notes` (a per-lot cosmetic annotation). NOT material: `p_notes` (free-text annotation) and
-- each line's `lot_notes` — cosmetic metadata the prompt's own §10 explicitly says not to compare.
-- The full material snapshot is stored verbatim as submitted (`idempotency_request`) rather than
-- reconstructed from the written rows afterward, so the comparison is exact and needs no second
-- "what would this have resolved to" computation — jsonb equality already ignores key order and
-- whitespace, so a client that resends the identical request object compares equal regardless of
-- how its JSON happens to be serialized.
--
-- CONTRACT (prompt §11). `p_idempotency_key` stays OPTIONAL at the database layer (`default null`,
-- unlike create_sale's required key) — this project has dozens of existing DB-test call sites for
-- create_purchase across tests/db/**, none of which pass a key, and forcing all of them to adopt
-- one is disproportionate churn for a purchase-specific reliability fix (docs/CLAUDE.md: smallest
-- complete solution, do not expand scope). A caller that omits the key gets exactly today's
-- behaviour (no replay protection). The REAL product path is not optional: the client data adapter
-- and PurchaseFormPage (src/data/purchases.ts, src/features/purchases/PurchaseFormPage.tsx) always
-- generate and send one, per the same "new UUID per fresh form instance, same UUID across a retry
-- of the same attempt, new UUID only after a successful commit" lifecycle SaleFormPage already
-- established — never regenerated merely because an error was shown.
--
-- Checked before any validation (same ordering reasoning as create_opening_from_provisional's own
-- comment): a retry of an otherwise-invalid request must also replay cleanly once the first attempt
-- has actually succeeded, and a retry must never re-run the ledger writes to find out whether it
-- would collide.
...
-- Partial (not a plain unique index, unlike sales' `idempotency_key uuid not null`): most existing
-- and many future purchases still carry no key at all (every caller that predates this migration,
-- and any caller that deliberately omits one), so NULLs must not collide with each other.
create unique index purchases_user_idempotency_key_idx
  on public.purchases (user_id, idempotency_key)
  where idempotency_key is not null;

-- A new parameter changes a function's identity — CREATE OR REPLACE is not available for this
-- (TESTING.md §6a / D-054). Drop and recreate.
--
-- TESTING.md §6a checklist, applied: the body below is 20260829120060_m11_create_purchase_
-- sealed_intent.sql's body (the LATEST prior version — confirmed via a full grep of every
-- migration that ever CREATE-OR-REPLACEd or DROP+CREATEd create_purchase; M10's own intermediate
-- rewrite, 20260828110000, is superseded by M11's and not used as the base here), not the original
-- M8 one — every M10/M11 fix (residual_nok_minor, the sealed-product existence/RLS check, per-line
-- sealed_intent written onto the LOT, manual_value_minor allowed for a sealed holding too, the
-- graded-card condition-null override) is preserved unchanged below; only the idempotency block and
-- the two new insert columns are new.
...
begin
  ...
  -- Built unconditionally (cheap) so both the compare-on-replay and the store-on-first-write paths
  -- below share exactly one definition of "material" — see the migration header for the field list
  -- and what is deliberately excluded (p_notes, each line's lot_notes).
  ...
  -- sequential check above (neither commits until after the other's SELECT ran), then both attempt
  -- this INSERT. The loser hits `purchases_user_idempotency_key_idx`. Same shape as P94's
  -- add_card_acquisition race fix (20260903120020): the outer BEGIN's implicit savepoint rolls back
  -- everything this losing transaction attempted (purchase, lines, holdings, lots), the handler
  -- re-reads the winner's committed row and applies the SAME material-equivalence check the early
  -- path already ran, so a race for a genuinely different request is still refused, not silently
  -- handed the winner's purchase.
  begin
  ...
  for v_idx in 0 .. v_line_count - 1 loop
    ...
    -- Per-line sealed intent, defaulted the same way add_card_acquisition's direct-add path
    -- defaults it (prompt §20 — organisational only, never a financial choice).
