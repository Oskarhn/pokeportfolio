-- M10 restates the complete privilege baseline (SECURITY.md §5.9) — required because this
-- milestone adds three new tables (sales, sale_lines, lot_disposals), five new functions
-- (allocate_largest_remainder_signed, create_sale, update_sale, void_sale, sales_summary), and one
-- new column on an existing table's UPDATE grant (acquisition_lots.residual_nok_minor,
-- 20260828110000). Everything else below is identical to
-- `20260827120010_m91_privilege_baseline.sql`; only the additions noted inline changed. CI selects
-- the lexicographically-latest `*_privilege_baseline.sql` automatically, so this file becoming the
-- newest one is what makes it "the" baseline from here.
...
-- function privileges — unchanged by M10.
