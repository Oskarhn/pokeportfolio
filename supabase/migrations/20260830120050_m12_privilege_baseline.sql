-- M12 restates the complete privilege baseline (SECURITY.md §5.9). This milestone adds three
-- tables (portfolio_snapshots — owner-SELECT-only derived cache; portfolio_recompute_queue and
-- portfolio_recompute_runs — service/internal-only, no browser grant at all) and six
-- browser-reachable functions: the four dashboard reads, plus two new service-only engine
-- functions that stay ungranted to browsers. Everything else below is identical to
-- `20260829120040_m11_privilege_baseline.sql`; only the M12 additions are noted inline.
...
-- function privileges — unchanged by M11/M12.
