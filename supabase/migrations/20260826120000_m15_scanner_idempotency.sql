...
--     fires a unique_violation; the exception handler catches it and replays the original result.
...
begin
  ...
  -- ── The acquisition lot itself (with idempotency on insert) ──────────────────────────────────
  -- Two concurrent calls with the same client_request_key can both pass the replay check above
  -- before either commits; the partial unique index is the arbiter — the loser's INSERT fires
  -- unique_violation, the exception handler catches it and returns the winner's row.
  begin
  ...
  exception when unique_violation then
    -- Without a client key the partial index cannot be what fired — re-raise.
    if p_client_request_key is null then
      raise;
    end if;
    -- The winner committed first; replay its result.
    select al.holding_id, al.id as lot_id into v_replay
      from public.acquisition_lots al
     where al.user_id = v_user_id
       and al.client_request_key = p_client_request_key;
...
comment on function public.add_card_acquisition(
  ...
  '(existing callers) behaves exactly as before.';
