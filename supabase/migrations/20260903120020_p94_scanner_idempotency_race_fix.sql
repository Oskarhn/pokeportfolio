-- P94 F-20: fixes a real concurrency gap in the M15 scanner idempotency function
-- (20260903120000_m15_scanner_idempotency.sql, which is NEVER edited in place per this
-- repository's discipline — this is a forward migration replacing the function body only).
--
-- THE BUG: add_card_acquisition has TWO replay paths that must agree —
--   1. the EARLY sequential check (runs before any mutation): selects the existing lot for
--      (user_id, client_request_key), and correctly rejects a voided lot ("idempotency-key-reuse")
--      before ever returning it, plus rejects a material mismatch.
--   2. the unique_violation EXCEPTION HANDLER (runs only when two concurrent calls race the same
--      key — the loser's own INSERT hits the partial unique index): selects the WINNER's committed
--      lot and returns it directly. This path never checked voided_at or material equivalence at
--      all — a genuine, if narrow, gap: if the winning lot is voided in the window between the
--      winner's commit and the loser's exception-handler lookup, the loser silently reports success
--      against a voided acquisition, resurrecting removed inventory from the caller's point of
--      view. F-21's companion gap: the same exception-handler path also never checked whether the
--      race was even for the SAME logical request (grader/grade material mismatch), so a losing
--      concurrent call with genuinely different material could also be handed back the winner's
--      lot as if it were its own.
--
-- THE FIX: the exception handler now runs the EXACT SAME voided_at check and material-equivalence
-- predicate the early path already runs, with the identical external error semantics
-- ('idempotency-key-reuse: ...'). CREATE OR REPLACE, not DROP+CREATE (D-054/D-059 discipline: a
-- DROP+CREATE is required only when the signature changes — it does not here, so grants persist
-- automatically and no revoke/grant statements are needed).
--
-- Everything else in the function body is byte-identical to the original migration.
...
comment on function public.add_card_acquisition(
  ...
  'P94 F-20/F-21: the race-path (unique_violation) replay applies the same voided_at and '
  'material-equivalence checks as the sequential early-check path.';
