-- M7.1 restates the complete privilege baseline, the same way each milestone's own baseline has
-- superseded the previous one (SECURITY.md §5.9) — required here because list_portfolio's old
-- signature was dropped and replaced (20260823120010_m71_number_sort.sql) and two new objects
-- exist (natural_sort_key, profiles.hide_values/use_eu_pricing). CI selects the
-- lexicographically-latest `*_privilege_baseline.sql` automatically, so this file becoming the
-- newest one is what makes it "the" baseline from here — no CI edit needed.
...
-- function privileges — unchanged by M7.1.
