-- P133 restates the complete privilege baseline (SECURITY.md §5.9). This is the first migration
-- since P108 to introduce genuinely NEW grants rather than CREATE OR REPLACE on an already-granted
-- signature: 20260915120000_p133_currency_exponent_fx_conversion.sql adds two internal SQL helper
-- functions (currency_minor_unit_exponent, money_minor_to_nok_minor — P130-02/D-132) that need
-- EXECUTE from `authenticated` (create_purchase/update_purchase are SECURITY INVOKER and reach them
-- as the caller's own role) and from `service_role` (the two frozen-rate CHECK constraints on
-- purchases/sales evaluate under whichever role performs the write, including a direct
-- service-role table write that bypasses every RPC — tests/db/money-boundary.test.ts is exactly
-- such a write). No existing signature changed.
-- Everything else below is identical to `20260905120020_p108_privilege_baseline.sql`; only the
-- P133 addition is new (marked inline).
...
-- P133 (P130-02/D-132): the currency-exponent lookup and the canonical exponent-aware NOK
-- conversion helper. Granted to authenticated AND service_role — see the migration header.
...
-- function privileges — unchanged since M11/M12/P28/P43/M16/P133.
