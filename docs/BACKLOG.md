...
| `tests/db/m10_sales.test.ts` identified purchase lines by array position after `.order('created_at')` — **FIXED (P108b)** | Found during a P108 stress-testing session: a 24-run `pnpm test:db` sweep (20 `--sequence.shuffle.files` seeds + 4 fresh-reset repeats) failed exactly once, at seed=20, on E7's hardcoded financial assertions — a value mismatch, not an exception. Root-caused directly, not by chasing a lucky repro: `create_purchase` writes every line for one call inside a single PL/pgSQL invocation, and Postgres's `now()` (= `transaction_timestamp()`) is frozen for the whole transaction — confirmed empirically that a 4-line purchase's `purchase_lines.created_at` values are byte-identical to the microsecond across all 4 rows. `purchaseLinesFor`'s `.order('created_at')` therefore has a permanent, guaranteed 4-way tie on every single invocation of this helper for a multi-line purchase; Postgres's tie-break order for equal sort keys is implementation-defined, not a documented guarantee, and empirically matches insertion order only "usually" — it silently returned a different order at least once across 24 real runs, which is exactly what happened. This was never a race with the M12 recompute cron or any other process; it is a structural test bug, reproducible in principle on ANY run, not merely "rare." Fixed the 3 genuinely at-risk multi-line call sites in this file (E2, E7, the lot-cost-adjustment test) with a `lineByPrice()` helper that finds a specific line by its (always-unique-in-these-fixtures) `unit_price_minor` instead of by array position — removes the dependency on tie-break order entirely rather than papering over it with a secondary sort key that would still be an arbitrary, semantically-meaningless order. Verified: `tests/db/m10_sales.test.ts` + its authorization counterpart 28/28, full `pnpm test:db` 604/604 (1 pre-existing skip), 5 additional shuffle-seed stress runs clean. **Not audited: whether this exact `.order('created_at')`-with-no-secondary-key pattern exists in OTHER test files outside this one** (out of this narrowly-scoped investigation's remit) — worth a follow-up grep across `tests/db/**` before assuming this defect class is fully closed. |
