...
## D-109 — robots.txt/sitemap default to "disallow everything, allow three pages" (P101)
...
## D-110 — No cookie-consent banner; nothing non-essential is stored client-side (P101)
...
- Nothing else. No analytics storage exists yet (see D-111): Cloudflare Web Analytics is, per
...
## D-111 — Cloudflare Web Analytics adopted, opt-in via env var, off by default (P101)
...
'fingerprint' individuals via IP address, User Agent, or any other data." That is what makes D-110's
